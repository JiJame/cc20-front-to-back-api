  ...
  try {
    ...
    */
    ...
    const user = await prisma.user.findFirst({
      where: {
        email: email, // email ตัวหลังมาจากที่ดีซัคเจอริ่ง
      },
    });
    console.log(user);
    if (user) {
...
export const login = async (req, res, next) => {
  ...
  */
    ...
    const checkPassword = bcrypt.compareSync(password, user.password); // pass ตัวแรกมาจาก req.body ที่พิมเข้ามา อีกตัวมากจาก DB
